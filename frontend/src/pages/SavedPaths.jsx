...
import { mockPaths, getPersonById } from '../data/mockData'
import DraftMessageModal from '../components/DraftMessageModal'
import { useToast } from '../components/Toast'

function PathCard({ path, nodes, onRemove, onDraftMessage }) {
  return (
    <div className="card card-hover" style={{ padding: '18px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
        <div>
          <div style={{ fontWeight: 600, fontSize: '14px', color: 'var(--text-primary)' }}>
            {path.from.first_name} → {path.to.first_name} {path.to.last_name}
          </div>
          <div style={{ fontSize: '12px', color: 'var(--text-secondary)', marginTop: '2px' }}>
            {path.to.title} · {path.to.company}
          </div>
        </div>
      ...
      <div className="path-chain" style={{ marginBottom: '14px' }}>
        {nodes.map((person, i) => (
          <div key={person.id} style={{ display: 'flex', alignItems: 'center' }}>
            ...
            {i < nodes.length - 1 && (
              <div className="path-edge-line" style={{ minWidth: '24px', top: '-6px' }} />
            )}
          </div>
        ))}
      </div>
...
export default function SavedPaths() {
  ...
  function handleDraftMessage(path) {
    setModal({ target: path.to, pathPeople: path.path.map(getPersonById).filter(Boolean) })
  }
    ...
    return (
      <div style={{ padding: '32px', maxWidth: '600px' }}>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: '14px', margin: '0 0 40px' }}>
          Save paths to revisit them later.
        </p>
        <div
          className="card"
          style={{
            padding: '60px 40px',
            textAlign: 'center',
            background: 'linear-gradient(135deg, rgba(124,110,224,0.05) 0%, transparent 100%)',
            borderStyle: 'dashed',
          }}
        >
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '14px', margin: '0 0 24px' }}>
            Find a path and save it to see it here.
          </p>
          <button className="btn-primary" onClick={() => navigate('/find-path')}>
            Find a path
          </button>
        </div>
      </div>
    ...
    <div style={{ padding: '32px', maxWidth: '680px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '14px', margin: 0 }}>
            {paths.length} saved connection path{paths.length !== 1 ? 's' : ''}
          </p>
        </div>
        <button
          className="btn-primary"
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {paths.map((p) => (
          <PathCard
            key={p.id}
            path={p}
            nodes={p.path.map(getPersonById).filter(Boolean)}
      ...
      {modal && (
        <DraftMessageModal
          ...
          onClose={() => {
            toast?.add(`Message drafted for ${modal.target?.first_name}`, 'success')
            setModal(null)
          }}
        />
      )}
